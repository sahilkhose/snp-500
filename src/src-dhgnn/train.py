...
def run():
    ...
    train_data_loader = torch.utils.data.DataLoader(training_set, batch_size=1, num_workers=0)

    validation_set = dataset.StockDataset(x_valid, y_valid)
    valid_data_loader = torch.utils.data.DataLoader(validation_set, batch_size=1, num_workers=0)

    testing_set = dataset.StockDataset(x_test, y_test)
    test_data_loader = torch.utils.data.DataLoader(testing_set, batch_size=1, num_workers=0)
